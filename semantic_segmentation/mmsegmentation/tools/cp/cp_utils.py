...
def preprocess_batch(model, batch):
    """Schickt Batch durch mmseg DataPreprocessor → float32, normalisiert, device-korrekt."""
...
def aps_scores_for_batch(logits: torch.Tensor, labels: torch.Tensor, randomized: bool = False):
    ...
    base[has_prefix] = cumsums[has_prefix, r[has_prefix]-1] # list of cumsum before correct class
...
def cp_quantile(scores_1d: torch.Tensor, alpha: float) -> float:
    """Split-CP Quantil mit (n+1)-Korrektur und ceil."""
    n = scores_1d.numel()
    if n == 0:

...
@torch.no_grad()
def cp_uncertainty_from_logits(logits: torch.Tensor, q_hat: float, num_classes: int):
    """
    logits: [B,C,H,W]
    q_hat: globales CP-Quantil
    returns:
      cp_unc: [B,1,H,W] in [0,1], 0 = 100% certain (set size = 1), 1 = 100% uncertainty (set size = num_classes)
    """
    ...
    # normalize
    cp_unc = (set_size.float() - 1) / (max(num_classes - 1, 1)) # if only one class, divide by 1
    return cp_unc  # [B,1,H,W]